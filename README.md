<img src="Doc/IMG/development.png" alt="imagen" width="250">

![GitHub last commit](https://img.shields.io/github/last-commit/DiegoDG-01/Andromeda-Whatsapp_BOT?color=red&style=flat-square&logo=github&logoColor=white)
![Platform](https://img.shields.io/badge/Platform-Linux-blue?style=flat-square&logo=linux&logoColor=white)
![Status](https://img.shields.io/badge/status-active-brightgreen?style=flat-square&logo=data:image/png;base64,)
![GitHub](https://img.shields.io/github/license/DiegoDG-01/DJango-Easy-Deployment?style=flat-square&logo=data:image/png;base64,)
![](https://camo.githubusercontent.com/d45ec39e3a5a0862d2f17f584e7ee6aa949bcf2d583a09e5641133157389eb34/68747470733a2f2f696d672e736869656c64732e696f2f62616467652f4d616465253230576974682d2545322539392541352d6666303030302e737667)


## 🌐 Sobre Easy-DJango-Deployment <a name = "about"></a>

Easy-Django-Deployment es un proyecto para desplegar una aplicación Django de manera rápida y sencilla en servidores linux (debian/ubuntu) para producción.

## ⚙️ Instrucciones <a name = "instructions"></a>

1.- Clonar el repositorio

```
git clone https://github.com/DiegoDG-01/DJango-Easy-Deployment.git
```

2.- Dar permisos de ejecución al script

```
sudo chmod +x Installer.sh
```

3.- Ejecutar el script

```
sudo ./Installer.sh
```

4.- Seguir las instrucciones del script

## 📝 Nota <a name = "notes"></a>

Es posible agregar este script a la raiz de tu proyecto y ejecutarlo desde ahí, de esta manera no es necesario clonar el repositorio.

## 💡 Referencias <a name = "references"></a>

- [instalar-django-server-easy ](https://github.com/DiegoDG-01/instalar-django-server-easy)


## 🤓 Contacto <a name = "contact"></a>

- [linkedin](https://www.linkedin.com/in/diego-dg/)
- [Instagram](https://www.instagram.com/diegodg_01/)

## ✌🏼 Agradecimientos <a name = "acknowledgement"></a>

- Icon ([Deployment icons created by Uniconlabs - Flaticon](https://www.flaticon.com/free-icons/deployment))